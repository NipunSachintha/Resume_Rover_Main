...
const CandidateRanking = () => {
  ...
  const handleJobPositionChange = (e) => {
    ...
    // If "All Positions" is selected, fetch candidates from all positions
    if (newPosition === "") {
      fetchAllCandidates();
    }
    // Otherwise, the useEffect that watches jobPosition will handle fetching for specific position
  };
  
  // Fetch candidates from all available positions and combine them
  const fetchAllCandidates = async () => {
    setLoading(true);
    setError(null);
    
    // Check if we have job positions to fetch from
    if (!jobList || jobList.length === 0) {
      setError("No job positions available to fetch candidates from.");
      setLoading(false);
      return;
    }
    
    try {
      setError("Fetching candidates from all positions...");
      
      // Create an array of promises to fetch candidates from each position
      const allCandidatesPromises = jobList.map(position => 
        axios.get(`https://resumerovermain-production.up.railway.app/candidates/${encodeURIComponent(position)}`)
          .then(response => {
            // Return the data with the position attached if available
            if (response.data && Array.isArray(response.data)) {
          ...
          .catch(() => []) // Return empty array if fetching for a specific position fails
      );
      
      // Wait for all requests to complete
      const results = await Promise.all(allCandidatesPromises);
      
      // Combine all candidates from all positions
      let allCandidatesData = [];
      results.forEach(positionCandidates => {
        if (Array.isArray(positionCandidates)) {
          allCandidatesData = [...allCandidatesData, ...positionCandidates];
        }
      });
      ...
      if (uniqueCandidates.length > 0) {
        ...
        const formattedCandidates = uniqueCandidates.map((candidate, index) => ({
          id: candidate.id || candidate._id || `candidate-${index + 1}`,
          name: candidate.name || candidate.candidate_name || "Unknown Candidate",
          email: candidate.email || candidate.candidate_email || "no-email@example.com",
          phone: candidate.phone || candidate.candidate_phone || "Not provided",
          is_verified: candidate.is_verified || false,
          ranking_score: candidate.score || candidate.ranking_score || 0.635234678,
          status: candidate.status || "in progress",
        ...
        console.log(`Successfully fetched ${sortedCandidates.length} candidates from all positions`);
      } else {
        setError("No candidates found across all positions.");
        setAllCandidates([]);
        setDisplayedCandidates([]);
      }
    } catch (err) {
      console.error("Error fetching candidates from all positions:", err);
      setError("Failed to load candidates from all positions. Please try again later.");
      setAllCandidates([]);
      setDisplayedCandidates([]);
    } finally {
      setLoading(false);
